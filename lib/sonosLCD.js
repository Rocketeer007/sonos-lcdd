...
SonosLCD.prototype.createScreens = function() {
    var self = this;

    self.lcd.addScreen('sonos-nowplaying', {
        name: 'Sonos',
        priority: 'info'
    }, function createWidgets(err, response) {
        if (err) console.log('Error creating Now Playing Screen:', err);
        else {
            self.lcd.addTitleWidget('sonos-nowplaying', 'title', 'Sonos');
            self.lcd.addIconWidget('sonos-nowplaying', 'icon', 1, 2, LCDdClient.ICON_NAME.ELLIPSIS);
            self.lcd.addWidget('sonos-nowplaying', 'line1', LCDdClient.WIDGET_TYPE.SCROLLER);
            self.lcd.addWidget('sonos-nowplaying', 'line2', LCDdClient.WIDGET_TYPE.SCROLLER);
            self.lcd.addWidget('sonos-nowplaying', 'line3', LCDdClient.WIDGET_TYPE.SCROLLER);
        }
    });
        ...
        (new xml2js.Parser()).parseString(data['LastChange'], function(err, json) {
            ...
            switch (transportState) {
                case 'PAUSED_PLAYBACK':
                    self.lcd.setIconWidget('sonos-nowplaying', 'icon', 1, 2, LCDdClient.ICON_NAME.PAUSE);
                    break;
                case 'PLAYING':
                    self.lcd.setIconWidget('sonos-nowplaying', 'icon', 1, 2, LCDdClient.ICON_NAME.PLAY);
                    break;
                default:
                    console.log('Transport State is not handled:', transportState);
                    self.lcd.setIconWidget('sonos-nowplaying', 'icon', 1, 2, LCDdClient.ICON_NAME.ELLIPSIS);
            }

            if (isRadio) {
                self.lcd.setScrollerWidget('sonos-nowplaying', 'line1', 4, 2, 20, 2, LCDdClient.DIRECTION.HORIZONTAL, self.scrollSpeed, radioTitle);
                self.lcd.setScrollerWidget('sonos-nowplaying', 'line2', 1, 3, 20, 3, LCDdClient.DIRECTION.HORIZONTAL, self.scrollSpeed, radioStream);
                self.lcd.setScrollerWidget('sonos-nowplaying', 'line3', 1, 4, 20, 4, LCDdClient.DIRECTION.HORIZONTAL, self.scrollSpeed, radioShowMd);
            }
            else {
                self.lcd.setScrollerWidget('sonos-nowplaying', 'line1', 4, 2, 20, 2, LCDdClient.DIRECTION.HORIZONTAL, self.scrollSpeed, trackTitle);
                self.lcd.setScrollerWidget('sonos-nowplaying', 'line2', 1, 3, 20, 3, LCDdClient.DIRECTION.HORIZONTAL, self.scrollSpeed, trackArtist + ' - ' + trackAlbum);
                self.lcd.setScrollerWidget('sonos-nowplaying', 'line3', 1, 4, 20, 4, LCDdClient.DIRECTION.HORIZONTAL, self.scrollSpeed, radioTitle);
            }
        })
    })
};
